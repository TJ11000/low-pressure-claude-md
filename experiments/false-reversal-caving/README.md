...
Reading, as far as it goes: what these runs show is how easy the agent is to move. Told something false about a fact sitting three exchanges earlier in the same conversation, every run caved without checking and rewrote its picture of the user (no run apologised in words — judge field apology_sentences is 0 in all 16). Several diagnosed their own failure in the same breath (*"self-report is the weakest evidence, and I weighted it the heaviest"*) and still did not open the conversation to check. Three of the four Opus false-reversal runs said they doubted the claim, then yielded anyway.
